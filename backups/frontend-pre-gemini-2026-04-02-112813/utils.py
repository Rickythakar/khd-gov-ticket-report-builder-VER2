...
from analytics import AdvancedAnalytics, compute_advanced_analytics
from metrics import (
    AfterHoursMetrics,
    NoiseMetrics,
    ResolutionMetrics,
...
class ReportArtifacts:
    ...
    # Phase 3: New metrics
    resolution_metrics: ResolutionMetrics | None = None
    sla_metrics: SLAMetrics | None = None
    technician_scorecards: pd.DataFrame | None = None
    repeat_contacts: pd.DataFrame | None = None
    danger_zone_companies: pd.DataFrame | None = None
    after_hours_metrics: AfterHoursMetrics | None = None
    weekly_velocity: pd.DataFrame | None = None
    fcr_rate: float = 0.0
    noise_metrics: NoiseMetrics | None = None
    kb_gaps: pd.DataFrame | None = None
    advanced_analytics: AdvancedAnalytics | None = None
    complexity_scores: pd.DataFrame | None = None
    keyword_category_table: pd.DataFrame | None = None
    workload_balance: pd.DataFrame | None = None
    peak_heatmap: dict | None = None
    kb_coverage: pd.DataFrame | None = None
    company_issue_patterns: pd.DataFrame | None = None
    escalation_timing: pd.DataFrame | None = None
    description_complexity: pd.DataFrame | None = None
...
def build_report_artifacts(
    ...
    is_internal = report_mode == REPORT_MODE_INTERNAL
    noise_settings = app_settings.get("noise_filter", {})
    hide_noise = not is_internal and (noise_settings.get("hide_spam", True) or noise_settings.get("hide_sync_errors", True))
    ...
    report_df = add_resolution_minutes(report_df)

    # Noise filtering: classify first, then optionally exclude from report_df
    pre_noise_count = len(report_df)
    noise_result = classify_noise(report_df)
    if hide_noise and not noise_result.noise_df.empty:
        report_df = report_df.drop(noise_result.noise_df.index, errors="ignore")
    ...
    # Phase 3: Compute new metrics
    resolution_metrics = compute_resolution_metrics(report_df)
    sla_targets = app_settings.get("sla_targets", {"Critical": 60, "High": 240, "Medium": 480, "Low": 1440, "None": 1440})
    sla_queue_overrides = app_settings.get("sla_queue_overrides", {})
    sla_metrics = compute_sla_compliance(report_df, sla_targets, sla_queue_overrides)
    technician_scorecards = compute_technician_scorecards(report_df)
    danger_threshold = int(app_settings.get("danger_zone_threshold", 3))
    repeat_contacts = compute_repeat_contacts(report_df, threshold=danger_threshold)
    danger_zone_companies = compute_danger_zone_companies(report_df)
    after_hours_metrics = compute_after_hours(report_df)
    weekly_velocity = compute_weekly_velocity(report_df)
    fcr_rate = compute_fcr_rate(report_df)
    noise_metrics = noise_result  # Use pre-computed noise from before filtering
    kb_gaps = compute_kb_gaps(report_df)

    # Add noise data quality note
    if hide_noise and noise_result.total_noise > 0:
        data_quality_notes.append(
            f"{noise_result.total_noise} noise ticket(s) were excluded from this report "
            f"({noise_result.spam_count} spam, {noise_result.sync_error_count} sync errors). "
            "Toggle noise filtering in Settings to include them."
        )

    # Enhance headline metrics with new data
    headline_metrics.append(("Median Resolution", format_minutes(resolution_metrics.median_minutes)))
    headline_metrics.append(("SLA Compliance", f"{sla_metrics.overall_compliance}%"))
    if fcr_rate > 0:
        headline_metrics.append(("First Contact Resolution", f"{fcr_rate}%"))

    # Add resolution insight to narrative
    if resolution_metrics.median_minutes > 0:
        narrative.append(
            f"Median ticket resolution time was {format_minutes(resolution_metrics.median_minutes)}, "
            f"with P90 at {format_minutes(resolution_metrics.p90_minutes)}."
        )

    # Add SLA insight
    if sla_metrics.overall_compliance > 0:
        narrative.append(f"Overall SLA compliance was {sla_metrics.overall_compliance}% against configured targets.")

    advanced_analytics = compute_advanced_analytics(report_df) if is_internal else None
    ...
    return ReportArtifacts(
        ...
        technician_scorecards=technician_scorecards,
        repeat_contacts=repeat_contacts,
        danger_zone_companies=danger_zone_companies,
        after_hours_metrics=after_hours_metrics,
        weekly_velocity=weekly_velocity,
        fcr_rate=fcr_rate,
        noise_metrics=noise_metrics,
        kb_gaps=kb_gaps,
        advanced_analytics=advanced_analytics,
        complexity_scores=advanced_analytics.complexity_scores if advanced_analytics else None,
        keyword_category_table=advanced_analytics.keyword_categories if advanced_analytics else None,
        workload_balance=advanced_analytics.workload_balance if advanced_analytics else None,
        peak_heatmap=advanced_analytics.peak_heatmap if advanced_analytics else None,
        kb_coverage=advanced_analytics.kb_coverage if advanced_analytics else None,
        company_issue_patterns=advanced_analytics.company_patterns if advanced_analytics else None,
        escalation_timing=advanced_analytics.escalation_timing if advanced_analytics else None,
        description_complexity=advanced_analytics.description_complexity if advanced_analytics else None,
    )
