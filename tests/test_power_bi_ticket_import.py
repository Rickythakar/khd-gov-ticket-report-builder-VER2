...
class PowerBiTicketImportTests(unittest.TestCase):
    ...
    def test_power_bi_export_is_transformed_into_canonical_schema(self) -> None:
        ...
        self.assertEqual(prepared.loc[0, "Created"], prepared.loc[0, "Complete Date"])
        self.assertTrue(any("Complete Date derived" in note for note in result.normalization_notes))
    ...
    def test_missing_power_bi_required_columns_are_rejected_cleanly(self) -> None:
        ...
        self.assertIn("Power BI ticket export columns", build_unsupported_upload_message(schema_result))
